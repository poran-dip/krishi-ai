    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-blue-50 to-indigo-100">
      ...
      <div className="relative z-10 min-h-screen p-6">
        <div className="max-w-4xl mx-auto">
          {/* Main Chat Container */}
          <div className="bg-white/70 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 h-[85vh] flex flex-col overflow-hidden">
            {/* Header */}
            <div className="p-6 bg-gradient-to-r from-emerald-600 via-green-600 to-blue-600 text-white relative overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-gradient-to-b from-gray-50/50 to-white/50">
              {messages.map((message) => (
                ...
                >
                  {message.type === "bot" && (
                    <div className="w-10 h-10 bg-gradient-to-br from-emerald-500 to-blue-500 rounded-xl flex items-center justify-center shadow-lg border-2 border-white flex-shrink-0">
                      <Bot className="text-white w-5 h-5" />
                    </div>
                  )}
                  
                  <div
                    className={`max-w-xs md:max-w-lg xl:max-w-2xl px-5 py-3 rounded-2xl shadow-lg border transition-all duration-200 hover:shadow-xl ${
                      message.type === "user"
                  ...
                  >
                    {message.type === "bot" ? (
                      <div className="prose prose-sm max-w-none prose-headings:text-gray-800 prose-p:text-gray-700 prose-strong:text-gray-900 prose-ul:text-gray-700 prose-li:text-gray-700">
                        <ReactMarkdown 
                          components={{
                            h1: ({children}) => <h1 className="text-lg font-bold mb-2 text-emerald-700">{children}</h1>,
                            h2: ({children}) => <h2 className="text-base font-semibold mb-2 text-emerald-600">{children}</h2>,
                            h3: ({children}) => <h3 className="text-sm font-semibold mb-1 text-emerald-600">{children}</h3>,
                  ...
                  {message.type === "user" && (
                    <div className="w-10 h-10 bg-gradient-to-br from-gray-600 to-gray-700 rounded-xl flex items-center justify-center shadow-lg border-2 border-white flex-shrink-0">
              ...
              <div className="flex gap-3">
                <div className="flex-1 relative">
                  <input
                    ...
                    className="w-full px-5 py-4 bg-white/90 backdrop-blur-sm border border-gray-300/50 rounded-2xl focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 transition-all duration-200 shadow-sm text-gray-800 placeholder-gray-500"
                    disabled={loading}
                  />
                </div>
                <button
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {[
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="px-3 py-2 text-xs bg-emerald-50 text-emerald-700 rounded-lg border border-emerald-200 hover:bg-emerald-100 transition-colors duration-200 disabled:opacity-50"
                  >
                    {suggestion}
                  </button>
                ))}
